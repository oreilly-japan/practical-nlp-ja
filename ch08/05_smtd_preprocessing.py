...
def process_URLs(transient_tweet_text):
    ...
    CommonTLDs = regex_or(
        "com", "co\\.uk", "org", "net", "info", "ca", "biz", "info", "edu", "in", "au"
    )
...
def process_EmailIds(transient_tweet_text):
    ...
    transient_tweet_text = re.sub(
        email_regex, " constantemailid ", transient_tweet_text
    )
...
def process_Mentions(transient_tweet_text):
    ...
    """
    transient_tweet_text = re.sub(
        r"@(\w+)", " constantnonbrandmention ", transient_tweet_text
    )
    return transient_tweet_text
...
def process_HashTags(transient_tweet_text):
    ...
    """
    transient_tweet_text = re.sub(
        r"#(\w+)\b", " constanthashtag ", transient_tweet_text
    )
    return transient_tweet_text
...
def process_BrandMentions(transient_tweet_text):
    ...
    transient_tweet_text = re.sub(
        BrandMentionRegex, " constantbrandmention ", transient_tweet_text
    )
    return transient_tweet_text
...
def process_BrandName(transient_tweet_text):
    ...
    transient_tweet_text = re.sub(
        BrandNameRegex, " constantbrandname ", transient_tweet_text
    )
    return transient_tweet_text
...
def identify_Savings(transient_tweet_text):
    ...
    transient_tweet_text = re.sub(
        sale_regex, " constantdiscount ", transient_tweet_text
    )
    # discount_List = []
...
def indentify_Offers(transient_tweet_text):
    ...
    transient_tweet_text = re.sub(
        r"(?<!#)\b(?:(up[\s]?to)?((rs|\$)*[\s]*(constantnum))[\s]*[%]?)?[\s]*[-|~|or]?[\.]?[\s]*((rs|\$)*[\s]*(constantnum))*[\s]*[%]?[\s]*(offer|off|cashback|cash|cash back)",
        " constantoffer ",
        transient_tweet_text,
    )
    transient_tweet_text = re.sub(
        r"(?<!#)\b(?:cashback|cash back|cash)\b",
        " constantoffer ",
        transient_tweet_text,
    )
    # Offer_List = []
...
def indentify_Promos(transient_tweet_text):
    ...
    transient_tweet_text = re.sub(
        r"(?<!#)\b(?:promocode|promo code|promo|coupon code|code)\b[\s]*(constantalphanum)\b",
        " constantpromo ",
        transient_tweet_text,
    )
    transient_tweet_text = re.sub(
        r"(?<!#)\b(?:promocode|promo code|promo|coupon code|code)\b[\s]*[a-z]+\b",
        " constantpromo ",
        transient_tweet_text,
    )
    transient_tweet_text = re.sub(
        r"(?<!#)\b(?:promocode|promo code|promo|coupon code|code)\b[\s]*[0-9]+\b",
        " constantpromo ",
        transient_tweet_text,
    )
    transient_tweet_text = re.sub(
        r"(?<!#)\b(?:promocode|promo code|promo|coupon code|code|coupon)[s]?\b",
        " constantpromo ",
        transient_tweet_text,
    )
    # Promo_List = []
...
def indentify_freebies(transient_tweet_text):
    ...
    transient_tweet_text = re.sub(
        freebies_regex1, " constantfreebies ", transient_tweet_text
    )
    freebies_regex2 = r"(?<!#)\b(?:free)[\s]+[a-z]*\b"
    transient_tweet_text = re.sub(
        freebies_regex2, " constantfreebies ", transient_tweet_text
    )
    return transient_tweet_text
...
def identify_AlphaNumerics(transient_tweet_text):
    ...
    """
    AlphaNumeric_regex = (
        r"(?<!#)\b(?:([a-z]+[0-9]+[a-z]*|[a-z]*[0-9]+[a-z]+)[a-z,0-9]*)\b"
    )
    transient_tweet_text = re.sub(
        AlphaNumeric_regex, " constantalphanum ", transient_tweet_text
    )
    return transient_tweet_text
